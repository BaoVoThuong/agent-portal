...
-- ===========================================================================
-- Time Off — self-service leave requests, US federal holidays, and approval.
-- US federal holidays are calculated in application code because observed dates
-- change every year. `time_off_holidays` holds additional company closures.
-- ===========================================================================
create table if not exists time_off_policies (
  ...
  updated_at timestamptz not null default now(),
  check (
    (counts_toward_balance and annual_allowance is not null and annual_allowance >= 0)
...
create table if not exists time_off_balances (
  ...
  policy_code text not null references time_off_policies(code) on delete restrict,
  leave_year integer not null check (leave_year between 2020 and 2200),
  -- NULL uses the policy default. HR can set a personal entitlement later.
...
-- Every manager-made credit/debit is retained by month for auditability. The
-- running total stays in time_off_balances so request validation remains fast.
create table if not exists time_off_balance_adjustments (
  ...
  created_by_id uuid not null references portal_account(id) on delete restrict,
  created_at timestamptz not null default now(),
  check (extract(year from effective_month) = leave_year)
);
create index if not exists time_off_balance_adjustments_account_idx
  on time_off_balance_adjustments (account_id, leave_year, effective_month desc, created_at desc);

create table if not exists time_off_holidays (
  id uuid primary key default gen_random_uuid(),
  holiday_date date not null unique,
  name text not null check (char_length(btrim(name)) between 1 and 120),
  created_by_id uuid references portal_account(id) on delete set null,
  created_at timestamptz not null default now()
);

create table if not exists time_off_requests (
  id uuid primary key default gen_random_uuid(),
  requester_id uuid not null references portal_account(id) on delete cascade,
  policy_code text not null references time_off_policies(code) on delete restrict,
  ...
  reviewer_id uuid references portal_account(id) on delete set null,
...
create index if not exists time_off_requests_requester_idx
  on time_off_requests (requester_id, created_at desc);

-- Approval serializes on the requester's policy/year balance row. That avoids
-- two approvers both seeing the same remaining balance and overspending it.
create or replace function approve_time_off_request(
  p_request_id uuid,
  p_reviewer_id uuid,
...
language plpgsql security definer set search_path = public as $$
declare
  request_value time_off_requests%rowtype;
  policy_value time_off_policies%rowtype;
  balance_value time_off_balances%rowtype;
...
begin
  ...
  if policy_value.counts_toward_balance then
    insert into time_off_balances (account_id, policy_code, leave_year)
    ...
    select * into balance_value
    from time_off_balances
    where account_id = request_value.requester_id
    ...
    if request_value.total_days > available_days then
      raise exception 'TIME_OFF_INSUFFICIENT_BALANCE';
    end if;
  end if;
...
revoke all on function approve_time_off_request(uuid, uuid, text)
  from public, anon, authenticated;
grant execute on function approve_time_off_request(uuid, uuid, text)
  to service_role;

-- Atomically updates the running balance and retains an immutable monthly
-- adjustment record. Authorization is enforced by the Next.js API boundary.
create or replace function adjust_time_off_balance(
  p_account_id uuid,
  p_policy_code text,
...
begin
  ...
  if not exists (select 1 from portal_account where id = p_account_id and is_active)
    or not exists (select 1 from portal_account where id = p_actor_id and is_active) then
    raise exception 'TIME_OFF_ACCOUNT_NOT_FOUND';
  end if;

  insert into time_off_balances (account_id, policy_code, leave_year, adjustment_days)
...
revoke all on function adjust_time_off_balance(uuid, text, integer, numeric, date, text, uuid)
  from public, anon, authenticated;
