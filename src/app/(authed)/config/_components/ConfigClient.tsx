...
export function ConfigClient({
  ...
  const optionRefreshSequenceRef = useRef(new Map<EnrollmentProgram, number>());
  ...
  async function refreshOptionData(program: EnrollmentProgram) {
  ...
  const enrollmentOptionsRefreshError =
    isEnrollmentProgram(scope) ? refreshErrors[`enrollment-options:${scope}`] : undefined;
        ...
        <div className="min-h-0 flex-1 overflow-hidden">
          ...
          {tab === "value" ? (
            <ConfigDropdownValuesSection
              ...
              optionSets={isEnrollmentProgram(scope) ? optionData[scope].sets : []}
              optionsBySet={
                isEnrollmentProgram(scope)
...
function requiredColumnTitle(
  ...
  if (!column.is_system && isEnrollmentProgram(scope)) {
...
function detailVisibilityDisabled(
  column: Pick<TableColumn, "is_system" | "scope" | "key" | "required">
): boolean {
  return (
    column.required ||
    (column.is_system && !supportsOptionalSystemDetail(column))
  );
}

function detailVisibilityTitle(
  column: Pick<TableColumn, "is_system" | "scope" | "key" | "required">
): string | undefined {
  if (column.required) {
    return "Required fields must always be reachable — turn off Required first.";
  }
  if (column.is_system && !supportsOptionalSystemDetail(column)) {
    return "This built-in field has a fixed detail behavior. Details can be configured only for custom fields and supported audit fields.";
  }
  if (column.is_system) {
    return "Show or hide this optional audit field in the Lead detail modal.";
  }
  return undefined;
}
  ...
  return (
    ...
    >
      ...
      <ToggleSwitch
        ...
        disabled={detailVisibilityDisabled(column)}
        title={detailVisibilityTitle(column)}
      ...
      : selected.kind === "custom"
        ...
        : selected.kind === "leadStatus"
          ...
          : selected.kind === "leadType"
            ...
            : sortEnrollmentOptions(optionsBySet[selected.setKey] ?? []).map((o) => ({
                id: o.id,
                label: o.label,
                color: o.color,
