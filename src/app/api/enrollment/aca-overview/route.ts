...
export async function GET(request: Request) {
  ...
  try {
    const snapshot = await fetchAcaOverviewSnapshot({ from, to, thresholdDays: url.searchParams.get("thresholdDays") });
    return NextResponse.json(snapshot);
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : "Could not load ACA overview." }, { status: 500 });
  }
}
