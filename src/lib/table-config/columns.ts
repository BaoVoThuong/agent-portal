...
// Lead details intentionally stay curated instead of showing every audit
// column. This makes a small, explicit exception for metadata an admin may
// choose to expose through the Details switch. Add future audit fields here
// only once their detail renderer exists.
const OPTIONAL_SYSTEM_DETAIL_KEYS: Record<TableScope, ReadonlySet<string>> = {
  cs: new Set(),
  aca: new Set(),
  medicare: new Set(),
  medicaid: new Set(),
...
export function supportsOptionalSystemDetail(
  column: Pick<TableColumn, "is_system" | "scope" | "key">
): boolean {
  return (
    column.is_system &&
    (OPTIONAL_SYSTEM_DETAIL_KEYS[column.scope]?.has(column.key) ?? false)
  );
}
