...
async function fetchAllRecords(program: EnrollmentProgram, from: string | null, to: string | null): Promise<AcaOverviewRecord[]> {
  ...
  for (let offset = 0; ; offset += PAGE_SIZE) {
    let query = supabase.from("enrollment_records").select(RECORD_COLUMNS, { count: "exact" }).eq("program", program).is("archived_at", null).order("created_at", { ascending: false }).range(offset, offset + PAGE_SIZE - 1);
...
async function fetchStageDwell(program: EnrollmentProgram, recordIds: readonly string[], now: Date): Promise<ReadonlyMap<string, number | null>> {
  ...
  for (let start = 0; start < recordIds.length; start += 500) {
    ...
    for (let offset = 0; ; offset += PAGE_SIZE) {
      const result = await supabase.from("enrollment_stage_cycles").select("stage_id,duration_seconds", { count: "exact" }).in("record_id", ids).eq("program", program).eq("kind", "dwell").eq("source", "live").gte("ended_at", cutoff).not("ended_at", "is", null).range(offset, offset + PAGE_SIZE - 1);
      if (result.error) throw new Error(result.error.message);
...
export async function fetchAttributedCycles(program: EnrollmentProgram, recordIds: readonly string[], now = new Date()): Promise<AttributedCycleRow[]> {
  ...
  for (let start = 0; start < recordIds.length; start += 500) {
    const ids = recordIds.slice(start, start + 500);
    for (let offset = 0; ; offset += PAGE_SIZE) {
      const result = await supabase.from("enrollment_stage_cycles").select("stage_id,responsible_start_email,responsible_end_email,duration_seconds", { count: "exact" }).in("record_id", ids).eq("program", program).eq("kind", "dwell").eq("source", "live").gte("ended_at", cutoff).not("ended_at", "is", null).not("responsible_start_email", "is", null).range(offset, offset + PAGE_SIZE - 1);
      if (result.error) throw new Error(result.error.message);
...
export async function fetchAcaOverviewSnapshot(params: { program?: EnrollmentProgram; from?: string | null; to?: string | null; thresholdDays?: string | null; now?: Date } = {}): Promise<AcaOverviewSnapshot> {
  ...
  const [dwell, attributedCycles] = await Promise.all([fetchStageDwell(program, recordIds, now), fetchAttributedCycles(program, recordIds, now)]);
