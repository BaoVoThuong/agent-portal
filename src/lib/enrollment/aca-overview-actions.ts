...
function running(input: AcaOverviewInput) {
  ...
  return input.records.filter((r) => !r.archived_at && !r.closed_at).map((record) => ({ record, stage: record.stage_id ? byId.get(record.stage_id) : undefined })).filter(({ stage }) => !stage || !isDashboardTerminal(stage));
}
