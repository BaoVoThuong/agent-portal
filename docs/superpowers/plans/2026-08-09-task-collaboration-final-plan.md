...
| 14. Atomic task create | C | P2 | Implemented | `TBD` | Typecheck pending | F12. Task, assignees, initial stage/assignment cycles, and `created` audit are now one idempotent RPC; rotation, notifications, broadcast, and reconciliation are warning-only after commit. Live DB fault/replay verification pending. |
