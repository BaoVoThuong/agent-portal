...
| 19. Cache TTL | D | P3 | Implemented | `91235d9` | Cache tests + typecheck + targeted lint passed | F16. Detail cache now expires in 5 minutes, supports explicit invalidation, and drawer reload returns a visible retry state without discarding committed detail. Browser realtime/cache-expiry verification pending. |
| 20. Operation limits | D | P3 | Implemented | `062bed5` | Limit tests + typecheck + targeted lint passed | F17. Added 10,000-character comments, 10 files/comment, 50MB aggregate, and client/server enforcement. Distribution audit could not run: `SUPABASE_URL`/`SUPABASE_SERVICE_ROLE_KEY` are unavailable in this environment; maxima are therefore unverified defaults. Malware scan remains out of scope; authenticated Office/PDF uploads need written risk acceptance before go-live. |
| 21. DB invariants | D | P3 | Implemented | `b4ad295` | Schema/script inspection + typecheck passed; live audit/write attempts unavailable without Supabase credentials | F18. Confirmed all application writers use atomic commands, added the narrow active comment-attachment index and a restricted audit RPC for cross-task/nested links. No triggers added because there is no non-command writer. Existing-data cleanliness and direct invalid-write rejection must be run against production before deployment. |
