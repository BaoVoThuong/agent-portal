...
| 20. Operation limits | D | P3 | Implemented | `TBD` | Limit tests + typecheck + targeted lint passed | F17. Added 10,000-character comments, 10 files/comment, 50MB aggregate, and client/server enforcement. Distribution audit could not run: `SUPABASE_URL`/`SUPABASE_SERVICE_ROLE_KEY` are unavailable in this environment; maxima are therefore unverified defaults. Malware scan remains out of scope; authenticated Office/PDF uploads need written risk acceptance before go-live. |
