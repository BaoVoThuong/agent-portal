...
| 22. Activity vocabulary | D | P3 | Implemented | `TBD` | Activity vocabulary tests + typecheck + targeted lint passed | F14. Extracted a testable label map covering every allowed task_activity type, added attachment/comment edit/delete labels, routed assignment wording through describeActivity, and removed dead branches for impossible event types. Unknown historical rows still degrade to raw text. |
