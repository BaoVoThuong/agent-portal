...
| 13 — notification enrichment | Completed | Added structured warning telemetry for each optional title/name/comment enrichment query while preserving fail-soft base notifications and avoiding PII in logs. | `git diff --check`; `npm run typecheck` | Pending commit |
