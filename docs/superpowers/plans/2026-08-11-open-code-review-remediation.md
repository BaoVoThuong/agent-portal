...
| 3 — health summary atomic replace | Completed | Added a service-role-only `replace_health_payment_summary(jsonb)` RPC in both schema artifacts, switched the health statement route to one transactional replacement call, and added scratch failure/success assertions. | `git diff --check`; static RPC/route review; scratch SQL added but not run because no DB URL is available in this environment | `30e66da` |
| 4 — staged data sync | Completed | Added run/staging tables, service-role-only begin/finalize/purge RPCs, safe target/RPC/batch validation, and switched the sync runner from delete-first REST writes to staged batches plus idempotent atomic promotion. | `git diff --check`; static runner/RPC review; scratch SQL added but not run because no DB URL is available in this environment | `f0b327d` |
| 5 — workbook limits | Completed | Added shared server-side workbook validation (extension/MIME, max 10 files, 25 MB/file, 100 MB/request), sequential parsing, and applied it to Health and all P&C workbook endpoints so no parser receives a rejected file. Hosting request-body cap remains a deployment verification item. | `git diff --check`; `npm run typecheck`; `npm run test:run -- src/lib/automation/workbook-upload.test.ts`; `npm run lint` (3 pre-existing CommentThread warnings, 0 errors) | `bc40d5d` |
| 6 — unique display keys | Completed | Added sequence-backed non-null unique `display_number` columns with deterministic null backfill for Tasks and Enrollment, replaced display derivation in list/detail/search/notification/export, added a 20,001-fixture scratch uniqueness test, and added a standalone rollout SQL for environments that have not applied the schema. | `git diff --check`; `npm run typecheck`; targeted sorting test; rollout SQL not run because no DB migration authority/connection is available | `247f8d0` + rollout pending |
| 7 — atomic Config agent delete | Completed | Added `delete_task_agent_atomic(text)` with normalized email/advisory lock, switched DELETE route to the RPC, and added idempotent scratch coverage. | `git diff --check`; `npm run typecheck`; scratch SQL added but not run because no DB URL is available in this environment | `7c37e19` |
| 8 — atomic option positioning | Completed | Added locked `create_table_column_option` RPC with explicit color/position validation, removed the ignored max-position read, and added scratch allocation coverage. | `git diff --check`; `npm run typecheck`; scratch SQL added but not run because no DB URL is available in this environment | `4b01543` |
| 9 — collaboration pagination | Completed | Added shared cursor pagination (50 comments/page) for Task and Enrollment detail, parent/deep-link expansion, bounded comment attachment queries, older-page UI with failure feedback, stable metadata counts, and per-file Enrollment signing fallbacks. | `git diff --check`; `npm run typecheck`; `npm run test:run -- src/lib/tasks/detail.test.ts src/lib/enrollment/comments.test.ts` (7 passed); 1,000-comment/browser/deep-link/storage-fault integration checks remain pending | `c7de64f` |
| 10 — Enrollment option snapshot | Completed | Added a program-scoped in-memory option map to the create request, removed per-field option reloads, and preserved wrong-set/archived/missing-option rejection and first-stage fallback behavior with unit coverage. | `git diff --check`; `npm run typecheck`; `npm run test:run -- src/lib/enrollment/options.test.ts src/lib/enrollment/form-options.test.ts` (5 passed) | `d5b2f37` |
| 11 — attachment deletion feedback | Completed | Added per-file deleting state, duplicate-click protection, parsed API/network errors, and accessible inline feedback while preserving rows on failed deletes and distinguishing post-delete refresh failures. | `git diff --check`; `npm run typecheck`; `npm run lint` (0 errors; 3 pre-existing CommentThread warnings) | `0d0f3cc` |
| 12 — data-quality semantics | Blocked on product decision | Not implemented: changing the `/2025` derivation or contradictory Health-row predicate without an owner decision could rewrite historical mart meaning or delete valid member-only rows. The required statement-year and row-retention fixtures/sign-off remain release gates. | Static code evidence re-checked; no safe implementation can be verified without product semantics and representative 2025/2026 fixtures | — |
