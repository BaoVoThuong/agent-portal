...
| 15 — realtime topic secret | Completed | Added `REALTIME_TOPIC_SECRET`/`AUTH_SECRET` resolution, production fail-fast behavior, non-production warning fallback, and HMAC/broadcast tests. | `git diff --check`; `npm run typecheck`; `npm run test:run -- src/lib/tasks/realtime.test.ts` (5 passed) | Pending commit |
