...
- Đưa Medicare Overview dùng chung dashboard shell và interaction với ACA, nhưng truyền `program=medicare` để lấy đúng records, stages, queue và stage cycles của Medicare.
- Bỏ implementation overview Medicare cũ bị trùng; không xoá dữ liệu, migration hoặc logic quyền của ACA.
- Chuẩn hóa nhận diện các outcome stage theo ý nghĩa (`ID card done`, `ID card unavailable`, `Terminated`) để số thứ tự stage khác nhau giữa ACA và Medicare vẫn hiển thị đúng.
- Bổ sung test riêng cho bộ stage Medicare và giữ tương thích khi API không truyền program bằng cách mặc định về ACA.

Commit: `dc8ced2`

Verification: 13 tests, typecheck, lint và production build đã pass.

### 2. Tối ưu giao diện Enrollment Overview
...
### 3. Làm nhẹ và làm rõ giao diện ngày tháng cho CS và Enrollment
...
### 4. Chuẩn hóa key hiển thị và tách bộ đếm Enrollment
...
### 5. Giữ Enrollment ở List sau khi tạo record
...
### 6. Đồng bộ quyền hiển thị Overview giữa Enrollment và CS
...
### 7. Đồng bộ quyền hiển thị task cho người tạo
...
### 8. Thông báo khi có người react comment
...
### 9. Đồng bộ trải nghiệm cộng tác của Enrollment với CS
...
### 10. Cho phép cộng tác trên task Backlog
...
### 11. Gộp comments và reactions vào pipeline Task Detail
...
### 12. Giảm chi phí polling và request nền
...
### 13. Đồng bộ Enrollment Overview với giao diện CS — Version 1
