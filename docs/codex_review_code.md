...
Last Updated: **2026-08-08 23:56 Asia/Ho_Chi_Minh**
Reviewed source through: **`2cd421d`** (execution log commits follow)
...
Regression Risk: Low.  
Verification: Direct UI/API comparison confirms the route writes `archived_at`; typecheck, targeted ESLint, and task detail tests (3 tests) pass. Browser copy/accessibility check remains.  
Status: **IMPLEMENTED — browser copy verification pending**
...
Fix: Route export failures through the existing typed Toast and use the Tasks screen's Vietnamese copy.
Regression Risk: Low.  
Verification: Both non-2xx and network-failure branches now call `setError`; `window.alert` no longer appears in Tasks. Typecheck, targeted ESLint, and Tasks tests (21 files / 243 tests) pass. Browser Toast/accessibility check remains.
...
Verification: Added quote/escape regression coverage for quote, backslash, comma, and operator-like identity input; Tasks tests pass (21 files / 243 tests), plus typecheck and targeted ESLint. Authenticated visibility checks remain.  
Status: **IMPLEMENTED — visibility regression verification pending**
...
Root Cause: Parent/activity write results were unchecked and the response always used a locally generated timestamp.
Impact: A successful comment can block the next legitimate edit and lose audit activity.  
Fix: Check parent/activity results, contain notification/broadcast failures as warnings, refetch the canonical record, and return only a persisted parent version.
Regression Risk: Medium.  
Verification: Parent-version resolution tests (3), `npm run typecheck`, targeted ESLint, and diff-check PASS. Authenticated comment failure-injection and next-edit verification remains.
Status: **IMPLEMENTED — failure-injection verification pending**
...
Fix: Equality-check the committed date before entering the per-record mutation queue; rapid real changes still serialize through M-02.
Regression Risk: Low–medium; test picker, typing, clearing, Required, and both programs.  
Verification: Drawer date change now skips a PATCH when the normalized value is unchanged; changed values use the existing per-record queue. Typecheck, targeted ESLint, and diff-check pass. Picker/manual-entry and rapid-network browser verification remain.
Status: **IMPLEMENTED — browser verification pending**
...
Root Cause: Topic had no program dimension.
Impact: Cross-program request amplification.  
Fix: Added program-scoped topics and subscriptions; known program mutations target only that topic, while config/cron fan-out remains explicit and the legacy topic is retained during rollout.
Regression Risk: Medium.  
Verification: Realtime topic tests (2), `npm run typecheck`, targeted ESLint, and diff-check PASS. Two-tab ACA/Medicare WebSocket/refetch-count verification remains.
Status: **IMPLEMENTED — browser realtime verification pending**
...
Fix: Enrollment create/update now fail closed with `SCHEMA_OUT_OF_DATE`/503 when the required description column is missing; read compatibility fallbacks remain until schema parity is verified.
Regression Risk: Low for verification, medium for removing compatibility fallbacks.  
Verification: Enrollment tests PASS (12 files / 50 tests), `npm run typecheck` PASS, targeted ESLint PASS, and `git diff --check` PASS. Production schema/RPC parity and migration health verification remain mandatory.
Status: **PARTIAL — write fail-closed implemented; schema parity gate remains open**
...
Fix: Store the hydrated serialized snapshot and skip identical saves; update the baseline only after a layout PUT succeeds.
Regression Risk: Low; verify real changes still persist.  
Verification: Enrollment now records the first post-hydration serialized layout and skips identical autosaves; user changes still go through the X-01 queue/version path. Typecheck, targeted ESLint, and diff-check pass. Browser request-count verification remains.
Status: **IMPLEMENTED — browser verification pending**
...
Fix: Render a labeled alert summary, mark invalid create fields for assistive discovery, and focus/scroll the first invalid control.
Regression Risk: Low.  
Verification: Missing Required fields now set a visible alert, the dialog renders API/create errors, and the first invalid field is focused/scrolled. Typecheck, targeted ESLint, and diff-check pass. Authenticated keyboard/screen-reader verification remains.
Status: **IMPLEMENTED — browser accessibility verification pending**
...
Fix: Share one strict calendar-date parser and return the same 400 validation error from create and update.
Regression Risk: Low–medium; inventory callers relying on coercion.  
Verification: `parseEnrollmentDate` now rejects malformed/impossible dates consistently while allowing explicit clears; create and update use it. Date helper tests pass (3 tests), Enrollment suite passes (5 files / 36 tests), typecheck, targeted ESLint, and diff-check pass. Authenticated route status checks remain.
Status: **IMPLEMENTED — route verification pending**
...
Fix: Enforce at least one active stage per program at archive time; terminal/QC and in-use record migration remain decision-gated follow-up work.
Regression Risk: High.  
Verification: Enrollment suite PASS (12 files / 50 tests), `npm run typecheck` PASS, targeted ESLint PASS, and `git diff --check` PASS. Direct last-stage archive and in-use-stage browser/DB verification remains.
Status: **PARTIAL — last-stage guard implemented; semantic migration remains open**
...
Fix: Return committed column/order changes with explicit `warnings` when a post-commit layout reset fails; atomic server operations and committed revision broadcast remain follow-up work.
Regression Risk: High.  
Verification: Table-config tests PASS (9 files / 42 tests), `npm run typecheck` PASS, targeted ESLint PASS, and `git diff --check` PASS. Failure-injection and multi-admin atomicity verification remains.
Status: **PARTIAL — post-commit truthfulness implemented; atomic transaction remains open**
...
Fix: Add `enrollment_option_usage_counts()` and consume grouped `{option_id, usage_count}` rows from Config instead of transferring every active Enrollment row.
Regression Risk: Low–medium.  
Verification: `npm run typecheck` PASS, Config ESLint PASS, Enrollment + table-config tests PASS (21 files / 92 tests), and `git diff --check` PASS. Schema apply and production volume/TTFB benchmark remain required.
Status: **IMPLEMENTED — schema/volume verification pending**
...
Fix: Serialize stage-rule PATCHes per option, disable that option's rule controls while pending, reconcile options after each mutation, and route feedback through the shared typed Toast runner.
Regression Risk: Medium.  
Verification: Table-config tests PASS (9 files / 42 tests), `npm run typecheck` PASS, targeted ESLint PASS, and `git diff --check` PASS.
Status: **IMPLEMENTED — stage-rule failure/double-toggle browser verification pending**
...
Root Cause: Feedback state omitted the result type.
Impact: Admin can misinterpret whether a global change applied.  
Fix: Added explicit success/error/info tone state and made option refresh failures observable to the shared action runner.
Regression Risk: Low.  
Verification: `npm run typecheck`, targeted ESLint, and diff-check PASS. Config success/failure visual and accessibility verification remains.
...
Fix: Keep the existing post-create semantics and disclose on ACA/Medicare custom Required controls that Create does not render custom inputs yet.
Regression Risk: Low for disclosure/disable; medium-high for adding Create inputs.  
Verification: `npm run typecheck` PASS, targeted ESLint PASS, table-config tests PASS (9 files / 42 tests), and `git diff --check` PASS.
Status: **IMPLEMENTED — browser copy verification pending**
...
Regression Risk: Low.  
Verification: `src/lib/table-config/columns.test.ts` PASS (16 tests), `npm run typecheck` PASS, targeted ESLint PASS, and `git diff --check` PASS.
Status: **IMPLEMENTED — direct POST/browser verification pending**
...
Root Cause: Eligibility was presentation-only; schema uses free text.
Impact: Work assigned to nobody, broken filters/notifications, hard-to-repair ownership.  
Fix: Validate changed ownership values against active `portal_account` rows; require `agent_email` to also be selected in `task_agents`, while preserving unchanged historical inactive values.
Regression Risk: Medium.  
Verification: Ownership helper tests (2 tests), `npm run typecheck`, targeted ESLint, and diff-check PASS. Authenticated create/update invalid-email route verification remains.
Status: **IMPLEMENTED — route verification pending**
...
Fix: Serialize/coalesce Tasks and Enrollment writes, return the canonical layout version, and make versioned PUTs conditional so stale tabs receive 409 instead of overwriting newer layout/reset state. Legacy PUTs without a version remain accepted for backward compatibility and must not coexist after rollout.
Regression Risk: Medium-high; all scopes share endpoint semantics.  
Verification: Conditional update/insert branches cover existing-row and reset/missing-row conflicts; Tasks and Enrollment writes queue/coalesce and carry `expected_updated_at`. Tasks + table-config suites pass (29 files / 282 tests), typecheck and targeted ESLint pass. Slow-network reordered PUT, admin reset, and two-tab staging verification remain.
Status: **IMPLEMENTED — staging race verification pending**
...
Fix: Add route-level table skeletons backed by one shared table-shaped fallback, while retaining the generic dashboard fallback for unrelated routes.
Regression Risk: Low.  
Verification: Added loading boundaries for Tasks, Enrollment, and Config; each renders the table toolbar/header/rows geometry. Typecheck, targeted ESLint, and diff-check pass. Browser navigation/layout-shift verification remains.
Status: **IMPLEMENTED — browser verification pending**
...
Fix: Give the Config new-column label input explicit light text/background and placeholder colors without changing the global theme contract.
Regression Risk: Low for targeted contrast fix.  
Verification: The input now explicitly sets `bg-white`, `text-[#172b4d]`, and placeholder color. Typecheck and targeted ESLint pass; visual dark-preference contrast check remains.
Status: **IMPLEMENTED — visual verification pending**
...
**17 issue groups:** T-10 through T-14; M-13 through M-17; C-13 through C-16; X-03 through X-05. T-10 through T-15 and X-03/X-04 are implemented pending browser/deployed evidence; the remaining P3 groups are open.
...
| 2026-08-08 | **C-09 — Config grouped Enrollment option usage.** Added a DB aggregate RPC and replaced the page's full active-record transfer with compact option usage counts. Follow-ups `d19dbb5` and `2cd421d` avoid a reserved SQL alias and restrict RPC execution to `service_role`. | `3ea385e`, `d19dbb5`, `2cd421d` | Enrollment + table-config tests PASS (21 files / 92 tests); `npm run typecheck` PASS; Config ESLint and diff-check PASS. Applying the RPC and production volume/TTFB benchmark remain required; local PostgreSQL was unavailable for replay. |
...
| 2026-08-08 | **C-04 — Config stale-client invalidation banner (partial).** Config writes now broadcast a dedicated topic; open Tasks/Enrollment pages show a non-destructive reload banner instead of silently keeping stale columns/options or resetting active input. Option-set producers use the Config topic while preserving program-scoped Enrollment refetches. | `76ef352`, `aea802e`, `fdab65f` | Tasks + Enrollment + table-config tests PASS (42 files / 335 tests); `npm run typecheck` PASS; targeted ESLint and diff-check PASS. Multi-session banner/reload verification and automatic scoped hydration remain. |
| 2026-08-08 | **Full regression verification after the execution batch.** | `fdab65f` | `npm run lint` PASS; `npm run test:run` PASS (**60 files / 458 tests**); `npm run build` PASS with all Next routes generated. This was rerun after the scoped Config broadcast correction. Browser, deployed-schema, failure-injection, scheduler, and production-volume gates remain explicitly open. |
...
Verification recorded across the execution commits through `2cd421d`:
...
- C-09 grouped Enrollment option usage: **Enrollment + table-config tests (21 files / 92 tests), typecheck, Config ESLint, and diff-check PASS**; RPC schema apply and production volume/TTFB proof remain.
